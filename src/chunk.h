...
typedef enum {
    ...
    OP_EXTRACT, // Pause the VM and put top value on the stack into VM.output
    ...
    OP_DEFINE_GLOBAL_STACK, // Define a global based off a value on the stack 1 byte
    ...
    OP_GET_GLOBAL_STACK, // Get a global based off of a value on the stack 1 byte
    OP_GET_GLOBAL_STACK_POPLESS, // Get a global value based off a value on the stack, but do not pop the "pointer", 1 byte
    ...
    OP_SET_GLOBAL_STACK, // Set a global variable based on a value on the stack, 1 byte
    ...
    OP_INDEX, // Access an index
    OP_INDEX_RANGE, // Access an index range
    OP_INDEX_RANGE_INTERVAL, // Access an index range with a custom interval
} OpCode;
